...
const transformCategories = async (data) =>{
  ...
  data.forEach(category => {
    ...
    transformedCategories.push({
      "name" : category.translated?.name || category.name,
      "streamId" : category.productStreamId,  
      "categoryid" : 'nl_' + category.id,
      "parents" : { 'categoryid' :  category.path ? 'nl_' + category.path.slice(1, -1).split('|').at(-1) : 31},
      "rank" : category.path ? category.path.slice(1, -1).split('|').length : 1,
    })
  })
...
exports = module.exports = __nccwpck_require__(2336);
exports.log = log;
...
exports.humanize = __nccwpck_require__(1964);
...
(__nccwpck_require__(7245).config)()
const xml2js = __nccwpck_require__(9253);

const getProducts = __nccwpck_require__(4256);
const getCategories = __nccwpck_require__(4029);
